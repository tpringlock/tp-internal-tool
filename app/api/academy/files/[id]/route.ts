...
/**
 * Streams a lesson's PDF attachment. Access is authorised by RLS: the select
 * returns the row only if the caller may see it (the course is published, or
 * the caller is an admin). The file is then fetched with the service role,
 * since the `academy` bucket is private.
 *
